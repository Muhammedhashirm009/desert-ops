...
@section('title', 'New Catalog Item — DessertOps')
@section('breadcrumb', 'New Catalog Item')

@section('content')
<div class="ph">
  <div>
    <div class="ph-title">New Catalog Item</div>
    <div class="ph-sub">Define a finished product that outlets produce by cooking half-prepared items</div>
  </div>
  <div class="ph-acts">
...
<form action="{{ route('outlet-catalog.store') }}" method="POST" id="catalog-form">
  @csrf
  ...
  <div class="row r-3-1" style="grid-template-columns: 1fr 340px; gap: 16px;">
    ...
    <!-- Right Panel: Catalog Item Details -->
    <div style="display: flex; flex-direction: column; gap: 16px;">
      <div class="card">
        <div class="ch">
          <div class="ch-title">Catalog Item Details</div>
        </div>
        <div class="cb">
          <div class="form-grp">
            <label for="name">Product Name *</label>
            <input type="text" name="name" id="name" class="form-input" required value="{{ old('name') }}" placeholder="e.g. Kunafa, Cheese Fatayer">
            @error('name')<span style="color: var(--red-tx); font-size: 12px;">{{ $message }}</span>@enderror
          </div>

          <div class="form-grp" style="margin-top: 16px;">
            <label for="sku">SKU Code *</label>
            <input type="text" name="sku" id="sku" class="form-input" required value="{{ old('sku') }}" placeholder="e.g. CAT-001">
            @error('sku')<span style="color: var(--red-tx); font-size: 12px;">{{ $message }}</span>@enderror
          </div>

          <div class="form-grp" style="margin-top: 16px;">
            <label for="retail_price">Retail Price (₹) *</label>
            <input type="number" step="0.01" name="retail_price" id="retail_price" class="form-input" required min="0" value="{{ old('retail_price') }}" placeholder="0.00">
          ...
          <div class="form-grp" style="margin-top: 16px;">
            ...
            <textarea name="description" id="description" class="form-input" rows="3" placeholder="Optional instructions...">{{ old('description') }}</textarea>
          </div>

          <div class="form-grp" style="margin-top: 16px;">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
              <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} style="width: 16px; height: 16px; accent-color: var(--btn);">
              <span>Active (available for production)</span>
            </label>
          </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @foreach($materials as $material)
      availableItems.push({ type: 'material', id: {{ $material->id }}, name: '{{ addslashes($material->name) }}', sku: '{{ $material->sku }}', unit: '{{ $material->unit ?? "units" }}' });
    @endforeach
    ...
    function createRow(preselect) {
        ...
        row.innerHTML = `
            <td>
              <select name="ingredients[${rowIndex}][id]" class="form-input ing-select" required style="height: 36px;">
                ${options}
              </select>
              <input type="hidden" name="ingredients[${rowIndex}][type]" class="ing-type-input" value="${defType}">
            </td>
            <td>
              <input type="number" step="0.01" name="ingredients[${rowIndex}][default_quantity]" class="form-input" required min="0.01" value="${defQty}" style="padding: 6px 8px;">
            </td>
            <td style="text-align: center;">
              <span class="ing-type-badge" style="font-size: 11px; padding: 2px 8px; border-radius: 4px; background: var(--bg2); color: var(--txt3);">${defType || '—'}</span>
            </td>
    ...
    document.getElementById('catalog-form').addEventListener('submit', function(e) {
        ...
        if (rows.length === 0) { e.preventDefault(); alert('Add at least one ingredient.'); }
    });
});
</script>
@endsection
