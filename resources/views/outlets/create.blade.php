...
@section('title', 'Add Retail Outlet — DessertOps')
@section('breadcrumb', 'Add Retail Outlet')

@section('content')
<div class="ph">
  <div>
    <div class="ph-title">Register Retail Outlet</div>
    <div class="ph-sub">Add a new company-owned store or franchise outlet</div>
  </div>
  <div class="ph-acts">
    <a href="{{ route('outlets.index') }}" class="btn-ghost">
      Cancel
    </a>
  </div>
</div>
...
<div class="card" style="max-width: 600px; margin: 0 auto;">
  <div class="ch">
    <div class="ch-title">Outlet Registration Details</div>
  </div>
  <div class="cb">
    <form action="{{ route('outlets.store') }}" method="POST">
      ...
      <div class="form-grp">
        ...
        <input type="text" name="name" id="name" class="form-input" required value="{{ old('name') }}" placeholder="e.g. Calicut Beach Outlet">
      ...
      <div class="grid-2">
        <div class="form-grp">
          ...
          <select name="type" id="type" class="form-input" required style="height: 38px;">
            <option value="own" {{ old('type') === 'own' ? 'selected' : '' }}>Company Owned</option>
            <option value="franchise" {{ old('type') === 'franchise' ? 'selected' : '' }}>Franchise Partner</option>
          </select>
        ...
        <div class="form-grp" id="commission-container" style="display: none;">
          ...
          <div style="display: flex; align-items: center; gap: 8px;">
            <input type="number" step="0.1" name="commission_rate" id="commission_rate" class="form-input" 
                   value="{{ old('commission_rate', '15.0') }}" min="0" max="100">
      ...
      <div class="grid-2" style="margin-top: 10px;">
        <div class="form-grp">
          ...
          <input type="text" name="contact_person" id="contact_person" class="form-input" value="{{ old('contact_person') }}" placeholder="Manager name">
          @error('contact_person')<span style="color: var(--red-tx); font-size: 12px;">{{ $message }}</span>@enderror
        </div>

        <div class="form-grp">
          <label for="phone">Phone Number</label>
          <input type="text" name="phone" id="phone" class="form-input" value="{{ old('phone') }}" placeholder="+91 ...">
      ...
      <div class="form-grp" style="margin-top: 10px;">
        ...
        <textarea name="address" id="address" class="form-input" rows="3" placeholder="Full physical address of the store...">{{ old('address') }}</textarea>
      ...
      <div style="margin: 20px 0 10px; padding: 16px; background: var(--bg2); border-radius: 8px; border: 1px solid var(--border);">
        ...
        <div class="grid-2">
          <div class="form-grp" style="margin-bottom: 0;">
            ...
            <input type="email" name="email" id="email" class="form-input" required value="{{ old('email') }}" placeholder="e.g. store@dessertops.com">
            @error('email')<span style="color: var(--red-tx); font-size: 12px;">{{ $message }}</span>@enderror
          </div>

          <div class="form-grp" style="margin-bottom: 0;">
            <label for="password">Password *</label>
            <input type="password" name="password" id="password" class="form-input" required placeholder="Minimum 4 characters">
