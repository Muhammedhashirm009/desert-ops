    ...
    <tbody>
      ...
      <tr class="{{ $material->is_low_stock ? 'row-low-stock' : '' }}">
        ...
        <td data-label="Material Name">
          ...
          <div class="td-meta">
            Unit: {{ $material->unit }}
            @if($material->category === 'packaging' && $material->per_box_qty)
              ({{ $material->per_box_qty }} pcs/box)
            @endif
          </div>
        </td>
        ...
        <td data-label="Store Stock" class="mono font-semibold" style="font-weight: 600;">
          {{ number_format($material->current_stock, 2) }} {{ $material->unit }}
        </td>
        <td data-label="Kitchen Stock" class="mono font-semibold" style="font-weight: 600; color: var(--blue-tx);">
          {{ number_format($material->kitchen_stock, 2) }} {{ $material->unit }}
        </td>
        <td data-label="Min Limit" class="mono td2">
        ...
        <td data-label="Suppliers">
          @if($material->suppliers_count > 0)
            <span class="badge bb">{{ $material->suppliers_count }}</span>
          @else
            <span style="color:var(--txt3); font-size:12px;">None</span>
          @endif
        </td>
      ...
      @empty
      <tr>
        <td colspan="9" class="text-center td2">No materials added yet. <a href="{{ route('materials.create') }}">Add one now</a>.</td>
      </tr>
      @endforelse
    </tbody>
  </table>
